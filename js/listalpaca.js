 $(document).ready(function () {
        	...
        	$.ajax({
			    ...
			    success: function (data, textStatus, xhr) {
			     ...
			     for (var i = 0; i < data.length; i++) {
			     	table.row.add([   
                       	'<td >'+data[i].name+'</td>','<td>'+data[i].weight+' kg</td>','<td><div style=\"background-color: '+data[i].color_hex+'; height:47px;\"></div></td>','<td >'+getFarmName(tempFarm, data[i].alpaca_farm_id)+'</td>','<td>'+data[i].shipping_cost+'</td>','<td ><input type="checkbox" data-id=\"'+data[i].id+'\" id=\"shippingStatus\" name=\"shippingStatus\" ></td>','<td><button id=\"DeleteButton\" class=\"delete_button\" type=\"button\" value='+data[i].id+'>X</button></td>']).draw();
